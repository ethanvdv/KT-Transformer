...
i = 0
for nu in range(N):     
    changes = Kaleidoscope.MKTkaleidoscopeIndexes(nu, N)
    
    output = Kaleidoscope.ApplyMKTransform(ph,changes)
    output.to('cuda')
    outputs = wandb.Image(np.abs(output[0, 0, :, :].cpu()), caption=f"nu {nu}")
    
    input = Kaleidoscope.newPseudoKT(output, changes)
    inputs = wandb.Image(np.abs(input[0, 0, :, :].cpu()), caption=f"nu {nu}")
    input.to('cuda')
    ...
    hhh = torch.count_nonzero(ph.to('cuda')-input.to('cuda')).to('cuda')
    if hhh == 0:
        wandb.log({"GoodTransforms": nu})
        nonLossyTransformsNu.append(nu)    
        
    
    wandb.log({"Error": hhh, "nu": nu})
    aarrayes[nu,nu] = hhh
...
# plt.ylabel('sigma')
# plt.colorbar()
plt.imsave(f'value1s.png', aarrayes, cmap='inferno', vmin=0, vmax=np.max(aarray))
