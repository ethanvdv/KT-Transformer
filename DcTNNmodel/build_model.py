...
# encList = [axVIT, patch2VIT, patchVIT]
# # Define the array of dictionaries
# encArgs = [axArgs, kd3Args, patchArgs]
device = 'cuda'
...
for epoch in range(epochs):
    ...
    for ph in dl:
        ...
        ph = rearrange(ph, 'b c h w -> c 1 b h w').contiguous()
        # for iter in range(batching):
        index = np.random.randint(1,ph.size(dim=0))
        singleimage = ph[index, :, :, :, :].to('cuda')
        # singleimage.to('cuda')
