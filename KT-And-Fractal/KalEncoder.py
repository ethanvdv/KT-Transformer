...
class patchVIT(nn.Module):
    ...
    def __init__(self, N, patch_size=16, kaleidoscope=False, layerNo=2, numCh=1, d_model=None, 
        ...
        for _ in range(layerNo):
            transformers.append(MKTEncoder(self.N, self.patch_size, numCh, kaleidoscope,
...
class MKTEncoder(nn.Module):
    ...
    """
    def __init__(self, image_size, nu, sigma, numCh=1, d_model=512, nhead=8, 
        ...
        num_patches = self.shift * self.shift
        ...
        if self.case == 1:
            ...
            self.mlp_head = nn.Sequential(
                ...
                Rearrange('(h w) c k1 k2-> c (h k1) (w k2)', h= self.N // self.nu, k1=self.nu, k2=self.nu),
            )
    ...
    def forward(self, img, src_mask=None):
        ...
        if self.case == 1:
            ...
            x = self.to_patch_embedding(x[0, :, :-1, :-1])
